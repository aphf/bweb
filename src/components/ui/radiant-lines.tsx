...
export type RadiantLinesOptions = {
  ...
  displacement?: number
  /**
   * Travel direction. `-1` drifts toward the center, `1` radiates outward.
   * Omit for scroll-driven behavior (down → outward, up → inward).
   */
  direction?: -1 | 1
  /**
...
export function createRadiantLines(
  ...
  const applyWarp = (v: number) => {
    ...
    // Locked direction prop wins; otherwise scroll down → outward, up → inward
...
export function RadiantLines({
  ...
  }, [colors, starCount, displacement, direction, containerRef])
